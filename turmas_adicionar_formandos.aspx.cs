...
{
    public partial class turmas_adicionar_formandos : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                if (!Page.IsPostBack)
                {
                    ...
                    lstb_formandos_elegiveis.DataSource = lst_formandos;
                    lstb_formandos_elegiveis.DataTextField = "nome_completo";
                    lstb_formandos_elegiveis.DataValueField = "cod_formando";
                    lstb_formandos_elegiveis.DataBind();
                }
            }
        }
        ...
        {
            if (lstb_formandos_elegiveis.SelectedIndex != -1)
            {
                Formandos formando_adicionado = new Formandos
                {
                    nome_completo = lstb_formandos_elegiveis.SelectedItem.ToString(),
                    cod_formando = Convert.ToInt32(lstb_formandos_elegiveis.SelectedValue)
                };

                ListItem index_formando_removido = lstb_formandos_elegiveis.SelectedItem;
                lstb_formandos_elegiveis.Items.Remove(index_formando_removido);
        ...
        protected void btn_remove_formandos_Click(object sender, EventArgs e)
        {
            if (lstb_formandos_turma.SelectedIndex != -1)
            {
                ...
                lstb_formandos_elegiveis.Items.Add(new ListItem(formando_removido.nome_completo, formando_removido.cod_formando.ToString()));
        ...
        protected void btn_salvar_alteracoes_Click(object sender, EventArgs e)
        {
            ...
            foreach (Formandos formando in lst_formandos)
                Turmas.Inserir_Formandos_Turma(formando.cod_formando, Convert.ToInt32(Request.QueryString["cod_turma"]), formando.cod_inscricao);

            lstb_formandos_elegiveis.Items.Clear();
            lstb_formandos_turma.Items.Clear();
            lbl_mensagem_formandos.Text = "";
            lbl_mensagem_formandos.CssClass = "";

            lbl_mensagem.Text = "Formandos adicionados à turma com sucesso! Será redirecionado para a página da Turma novamente.";
